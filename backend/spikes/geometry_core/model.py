"""Geometry Core Proof Spike — data model.

SPIKE CODE. Deliberately isolated from `app/`. Nothing here is production, nothing imports
production modules, and production must not import this. It exists to prove or disprove one
claim from PRIVATE_HOUSE_V1_ENGINE_DECISION.md §3 + §7:

    slicing tree + shape curves + wall thickness known BEFORE dimensioning

This module also carries the three corrections the reviewer required before the spike:
