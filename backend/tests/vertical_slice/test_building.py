...
def _design(plot, footprint, *, gross=None, net=None, entrance_zone="HALL") -> GeometricDesign:
    """A `GeometricDesign` with only what the building layer reads: plot, footprint, areas, entry."""
    x, y, w, h = footprint
    ...
    return GeometricDesign(
        ...
        gross_area_m2=round(w * h, 2) if gross is None else gross,
        net_area_m2=round(w * h * 0.9, 2) if net is None else net,
        wall_iterations=1,
    )
...
def _two_levels(ground_fp, upper_fp, *, plot=(0.0, 0.0, 20.0, 24.0), upper_gross=None):
    ...
    ground = LevelPlan(Level("L0", 0, LevelKind.GROUND),
                       ...
                       _design(plot, ground_fp), ValidationReport())
    upper = LevelPlan(Level("L1", 1, LevelKind.UPPER, elevation_m=FLOOR_TO_FLOOR_M),
                      ...
                      _design(plot, upper_fp, gross=upper_gross), ValidationReport())
    return Building(levels=(ground, upper), massing=Massing(plot, (ground_fp, upper_fp)),
                    cores=(core,))
...
def test_v2_holds_when_the_upper_outline_is_inside_the_ground_outline():
    ...
    assert next(c for c in report.checks if c.check_id == "V2").detail == "1 upper outline(s) contained"
...
def test_v2_fails_on_a_cantilever():
    ...
    assert "L1 outline" in failed[0].detail and "leaves L0" in failed[0].detail
...
def test_levels_are_ordered_unique_and_entered_the_right_way():
    ...
    with pytest.raises(ValueError, match="one outline per level"):
