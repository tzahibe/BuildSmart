...
1. **The generic row-sharing solution works, and both TOILET's and BATHROOM's pre-existing behaviour are exactly unchanged.** `_ROW_RESCUE_ROLES = (ProgramRole.TOILET, ProgramRole.LAUNDRY)` — named explicitly after measurement showed the fully general `ZoneGroup.SERVICE` form also touched a lone BATHROOM in 1 of 404 real briefs. Re-verified against the same 418-context corpus after narrowing: **0/404** planned briefs changed, in either status or room signature. `hub_bound`'s own `ZoneGroup.SERVICE` use (§4) was NOT narrowed — it is inert for any laundry=NONE brief by construction, not measurement, so there was nothing there to fix.
...
**KEEP GATED.** `LAUNDRY_ROOM_ENABLED` stays `False`; nothing here reaches a real user. The engineering is sound — generalised correctly, regression-tested against both the unit suite and the real 418-context corpus to a literal zero-change bar, hub verified rather than assumed, access/validation verified end to end. The regression question (§3) is closed. What is NOT yet decided is a product question this phase was not asked to answer:
