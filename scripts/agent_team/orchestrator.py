...
class Orchestrator:
    ...
    def _complete_worker(self, store: StateStore, issue_id: int, contract: IssueContract, spec: AgentRunSpec,
        ...
        if not result.ok:
            if self.rate_limited_run(result):
                ...
                work_reports.record_failure(
                    store, self.config, issue_id, stage="usage", failure_class="RATE_LIMITED",
                    attempt=rec.attempt_number, attempts_left=self.config.max_repair_attempts - max(0, rec.attempt_number - 1),
                    root_cause=(result.error or "rate limited")[:500], evidence_ref=run_record, next_action="paused")
                if not self.paused():
    ...
    def owner_reject(self, issue_id: int, *, source: str, owner_id, command_id: str, reason: str) -> dict:
        ...
        text = work_reports.record_failure(store, self.config, issue_id, stage="owner", failure_class="OWNER_REJECTED",
                                           attempt=rec.attempt_number, attempts_left=0, root_cause=reason,
                                           evidence_ref=None, next_action="blocked")
        self._milestone(issue_id, text)
    ...
    def owner_change_request(self, issue_id: int, *, source: str, owner_id, command_id: str, feedback: str) -> dict:
        ...
        self._set_state(store, issue_id, sm.FIX_REQUIRED, note="owner change request", failure_class="OWNER_CHANGE_REQUEST",
                        last_error=feedback[:1000])
        attempts_left = self.config.max_repair_attempts - max(0, rec.attempt_number - 1)
        text = work_reports.record_failure(store, self.config, issue_id, stage="owner", failure_class="OWNER_CHANGE_REQUEST",
                                           attempt=rec.attempt_number, attempts_left=attempts_left, root_cause=feedback,
                                           evidence_ref=None, next_action="repair")
