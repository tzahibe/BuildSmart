...
def render(config: Config, store: StateStore, resources: ResourceManager, *, probe_machine: bool = True, now: float | None = None) -> str:
    ...
    for r in by_state.get(sm.QUEUED, []):
        ...
        waiting_rows.append(f"#{r.issue_id} {_dom(r)} / {r.resource_class} / {why} — {r.title[:50]}")
    for r in by_state.get(sm.CLAIMED, []):
        waiting_rows.append(f"#{r.issue_id} {_dom(r)} / claimed, preparing worktree — {r.title[:50]}")
    for r in by_state.get(sm.FIX_REQUIRED, []):
        waiting_rows.append(f"#{r.issue_id} {_dom(r)} / repair pending ({r.failure_class}) — {r.title[:50]}")
    section("WAITING", waiting_rows)
    section("CI", [f"#{r.issue_id} PR #{r.pr_number} / {r.state.lower()} / {_age(r.updated_at, now)} — {r.title[:50]}"
    ...
    for r in by_state.get(sm.REVIEW, []):
        ...
        review_rows.append(f"#{r.issue_id} PR #{r.pr_number} / {r.risk} / review {verdict} — {r.title[:50]}")
    for r in by_state.get(sm.MERGED, []):
        review_rows.append(f"#{r.issue_id} PR #{r.pr_number} / merged by the owner, smoke pending — {r.title[:50]}")
    ...
    section("BLOCKED", [f"#{r.issue_id} / {r.failure_class or 'BLOCKED'} / {(r.last_error or '')[:80]} — {r.title[:50]}"
...
def render_compact(config: Config, store: StateStore, resources: ResourceManager, *, probe_machine: bool = False, now: float | None = None) -> str:
    ...
    for r in by.get(sm.FIX_REQUIRED, []):
        ...
        waiting.append(f"#{r.issue_id} {r.title[8:45].strip()} / ממתין לתיקון ({r.failure_class}){cause}")
