...
each candidate tree and each of the six representative outlines it enumerates every free sizing
decision and every admissible foot-band order on a coarse grid, builds the rooms as rectangles and
checks, on those rectangles, the three facts the gates measure: geometry (template minimums,
lobby caps, habitable rooms on the envelope), access (≥ 1.10 m of shared edge with the lobby for
every room that needs a door; ensuite beside its master) and wet adjacency (M5's rule). No
production code changed; no threshold tuned — the 80 % wet gate and the 1.35/1.40 aspect gates
are §6's.

**Candidates.** T1 = v2 (stacked flank). T3 = no stacking, bath beside the lobby in the middle
band. T4 / T4b = hybrid wet stack (bath over the ensuite beside the lobby, master under both),
with two flank bedrooms / with the second bedroom at the foot. T5 = guest bath at the lobby's head
in the front band. T2 = side-by-side, control only.
...
| **T1** best bed-class aspect, wet ≥ 80 % | **1.65** ✗ | **2.38** ✗ | **1.32** ✓ | **1.27** ✓ | **1.27** ✓ | **1.42** ✗ |
| T1 master / safe at that layout | 1.16 / 1.64 | 1.78 / 2.38 | 1.32 / 1.28 | 1.14 / 1.27 | 1.27 / 1.26 | 1.25 / 1.42 |
...
**3 bedrooms + safe room + 3 wet rooms**, T1: seats 6/6 on every outline but wet adjacency tops out
at **67 %** (the third wet room stacks under the second flank bedroom with nothing wet beside it)
and, with both flanks stacked, the shape bound is 2.00 / 2.71 / 1.75 / 1.56 / 1.17 / 1.94 — only
10 × 20 passes. The other trees fail access harder with the sixth door: T3 and T4 seat 5/6 on
every outline, T4b 3/6; T5 fits only 18 × 12 (1.81, wet 0 %); T2 fails exposure everywhere.

**What the seats prove.** A band can seat at most two door-needing rooms on the lobby's edge
(each needs 1.10 m of a ≤ 3.6 m edge and a room between them would need ≥ 2.6 m), and a flank
bedroom loses its door the moment a bath sits between it and the lobby. So for the 5-door brief a
three-band tree has exactly four seats — two flanks, two at the foot — unless it stacks (T1) or
puts a room at the lobby's head (T5, which only fits an 18 m front and isolates the bath: 0 %
wet). Every "no stacking" or "wet cluster beside the lobby" variant therefore fails access by
construction, not by sizing. T1 is the unique three-band tree that seats five and clusters the
wet rooms — and its stack is precisely what costs the lobby band 4.6 m of depth and breaks the
bedroom shapes on the wide outlines (§6).

**Decision gate: no topology passes geometry + access + wet adjacency across the representative
set.** T1 passes all three on the narrow-deep outlines (13 × 15, 12 × 18, 10 × 20) and fails the
shape gate on every outline ≥ 14 m wide; the only tree that could re-shape the wide outlines
(T5) is infeasible on five of six. **Recommendation:** not a new topology. Spec 007, if written,
should be a *policy* — offer the hub parti only where this bound says the outline can pass (the
computed feasibility, not a tuned width threshold), and leave the wide-shallow outlines to the
front-band/spine partis, whose bedrooms already sit at 1.31 (§6 table) — plus, for 3-wet briefs,
the finding that T1 cannot reach the wet gate at all. Not started; stopped for review.
