...
**Testing**: pytest (`backend/tests/vertical_slice/test_concept_generator.py`, `backend/tests/test_demo_p0.py`); measurement with `ab.py --toggle hub`, `snapshot.py`, `quality_metrics.py --split-by-strategy`, `hub_rooms.py`
...
**Scale/Scope**: one module (`concept_generator.py`: one new pure function + the ordering step in `generate_concepts`), two to three tests, RESULTS write-up
...
backend/
├── app/vertical_slice/
│   └── concept_generator.py        # + hub_bound() (pure), + eligibility ordering in generate_concepts,
│                                   #   + rationale/rejection text carrying the bound
├── spikes/failure_log_sweep/
│   ├── hub_topology_bound.py       # Phase 0 tool (already present) — the reference model
│   ├── ab.py / snapshot.py / quality_metrics.py / hub_rooms.py   # measurement, unchanged
│   └── README.md
└── tests/vertical_slice/
    └── test_concept_generator.py   # + eligibility tests (wide vs narrow outline; 3-wet; ordering; twin)
specs/005-hub-private-wing/RESULTS.md   # §8: 008 measurement
```

**Structure Decision**: single backend module change; the bound is a pure function next to `_hub_concept` so the harness tool and the engine share one model (the tool imports it after implementation, replacing its own T1 copy — see research R2).
